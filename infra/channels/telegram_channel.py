...
_POLLING_RESTART_BASE_DELAY_S = 5.0
_POLLING_RESTART_MAX_DELAY_S = 30.0
...
class TelegramChannel:
    ...
    def __init__(
        ...
        self._polling_restart_task: asyncio.Task[None] | None = None
        self._polling_network_errors = 0
    ...
    async def stop(self) -> None:
        self._stopping = True
        if self._polling_restart_task and not self._polling_restart_task.done():
            self._polling_restart_task.cancel()
            await asyncio.gather(self._polling_restart_task, return_exceptions=True)
    ...
    def _on_polling_error(self, exc: TelegramError) -> None:
        ...
        if isinstance(exc, Conflict):
            ...
            return
        logger.warning("[telegram] polling 异常: %s", exc)

        if not self._is_recoverable_polling_error(exc):
            logger.warning("[telegram] polling 异常不可自动恢复，交给框架处理")
            return

        self._polling_network_errors += 1
        if self._polling_restart_task and not self._polling_restart_task.done():
            logger.debug("[telegram] polling 自动恢复任务已存在，跳过重复调度")
            return

        delay = min(
            _POLLING_RESTART_BASE_DELAY_S * self._polling_network_errors,
            _POLLING_RESTART_MAX_DELAY_S,
        )
        logger.warning(
            "[telegram] polling 网络异常累计 %s 次，将在 %.1fs 后自动重启 polling",
            self._polling_network_errors,
            delay,
        )
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            logger.warning("[telegram] 无运行中的 event loop，无法调度 polling 自动恢复")
            return
        self._polling_restart_task = loop.create_task(
            self._restart_polling_after_backoff(delay),
            name="telegram_polling_restart",
        )

    @staticmethod
    def _is_recoverable_polling_error(exc: Exception) -> bool:
        """NetworkError/TimedOut 及其底层 httpx 超时/连接错误都尝试恢复。"""
        if isinstance(exc, (NetworkError, TimedOut)):
            return True
        name = type(exc).__name__.lower()
        return "connect" in name or "timeout" in name or "network" in name

    async def _restart_polling_after_backoff(self, delay: float) -> None:
        """网络抖动后重启 Telegram long polling，避免接收链路半失效。"""
        try:
            ...
            updater = self._app.updater
            if updater is None:
                logger.warning("[telegram] polling 自动恢复失败：updater 未初始化")
                return

            logger.warning("[telegram] polling 自动恢复开始")
            if updater.running:
                await updater.stop()
            ...
            self._polling_network_errors = 0
            logger.info("[telegram] polling 已自动恢复")
        except Conflict as e:
            self._on_polling_error(e)
        except asyncio.CancelledError:
            raise
        except Exception as e:
            logger.exception("[telegram] polling 自动恢复失败: %s", e)
        finally:
            if asyncio.current_task() is self._polling_restart_task:
                self._polling_restart_task = None
