...
def _load_channels_config(data: dict) -> ChannelsConfig:
    ...
    if feishu_data := channels_data.get("feishu"):
        app_id = _normalize_optional_config_text(
            _resolve(str(feishu_data.get("app_id", feishu_data.get("appId", ""))))
        )
        ...
        if bool(feishu_data.get("enabled", True)) and app_id and app_secret:
            feishu = FeishuChannelConfig(
                ...
                receive_id_type=str(
                    feishu_data.get("receive_id_type", "chat_id") or "chat_id"
                ),
                channel_name=str(feishu_data.get("channel_name", "feishu") or "feishu"),
            )
